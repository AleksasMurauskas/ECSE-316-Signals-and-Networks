...
//			}
		...
		ResponseResults rr= new ResponseResults(incomingPacket);
		
		if (rr.QR != 1) {
			System.out.println("Packet ERROR: Recieved packet is not a response.");
			System.out.println(rr.dataRecieved[3]);
			System.exit(1);
		}
		if(rr.RA != 1){
		...
		boolean auth=false;
		if (rr.AA == 1) {//Checks if packet is authoratative 
			auth = true;
		}
		...
		System.out.println("*** Answer Section (" + rr.ANCOUNT + " records) ***");
		...
		System.out.println("*** Additional Section (" + rr.ARCOUNT + " records) ***");
		if(rr.ARCOUNT<=0){
			System.out.println("NO Additional Records found"); //no additional records found
		}
		else{
			for(int x=0;x<rr.ARCOUNT;x++){
				current_record_size = readRecord(incomingPacket,response_loc,authorization)+12;//???????????????
				response_loc+=current_record_size;
			}
		}
	...
	public static int readRecord(DatagramPacket pack, int loc, String auth){
		...
		if(type.equals("A")){
			...
			System.out.println("NS\t" + extractDNSCompresion(pack, loc+ 12, 0) + "\t" + ttl_val + "\t" + auth);
		}
		else if(type.equals("CNAME")){
			System.out.println("CNAME\t" + extractDNSCompresion(pack, loc+ 12, 0) + "\t" + ttl_val + "\t" + auth);
		}
		else if(type.equals("MX")){
			short pref =  (short) ((resp_data[loc+12] << 8) | (resp_data[loc+13] & 0xFF));
			System.out.println("MX\t" + extractDNSCompresion(pack, loc+ 14, 0) + "\t" + ttl_val + "\t" + auth);
		}
		short read_len =(short) ((resp_data[loc+10] << 8) | (resp_data[loc+11] & 0xFF)); 
		return read_len;
	}

	public static String extractDNSCompresion(DatagramPacket pack, int offset, int cnt ){
		String name ="";
